...
class KernelMachine(nn.Module):  # inheriting from nn.Module!
    ...
    def fourier_features(self, x):
        ...
        fourier_features = torch.sqrt(torch.tensor(2/self.num_features_))*torch.cos(
            torch.mm(self.a_, x.transpose(1, 0)) + self.b_.unsqueeze(1)).transpose(1, 0).reshape(-1, 1)
        ...
        return torch.sparse_coo_tensor(index, fourier_features.repeat_interleave(
            self.match_dim_, 0).squeeze(1), (num_points, self.match_dim_, self.num_features_*self.match_dim_)).to_dense()

    def forward(self, x):
        # check sparse tensor problem
        return self.prediction_(self.fourier_features(x)).squeeze(2)
